...
start_osd() {
  ...
  # ceph-disk activiate has exec'ed /usr/bin/$BASE_OSD ${CLI_OPTS[@]} -f -i ${OSD_ID}
  # wait till docker stop or ceph-osd is killed
  OSD_PID=$(ps -ef |grep "$BASE_OSD" |grep osd."${OSD_ID}" |awk '{print $2}')
  if [ -n "${OSD_PID}" ]; then
  ...
  if [ "$mode" = "forego" ]; then
   echo "${CLUSTER}-${OSD_ID}: /usr/bin/$BASE_OSD ${CLI_OPTS[*]} -f -i ${OSD_ID} --setuser ceph --setgroup disk" | tee -a /etc/forego/"${CLUSTER}"/Procfile
  else
   log "SUCCESS"
   exec /usr/bin/"$BASE_OSD" "${CLI_OPTS[@]}" -f -i "${OSD_ID}" --setuser ceph --setgroup disk
  fi
}
